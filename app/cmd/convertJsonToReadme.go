...
var convertJsonToReadmeCmd = &cobra.Command{
	...
	Short: "Converts generated Game JSON files into a repository README.md",
	Long: `The source of truth are our YAML files in games/..
...
func cmdConvertJsonToReadme(cmd *cobra.Command, args []string) error {
	...
	// Sort list by name
	log.Printf("Sorting %d active games by name", len(games))
	...
	err = t.Execute(readmeTarget, games)
	if err != nil {
		return err
	}
